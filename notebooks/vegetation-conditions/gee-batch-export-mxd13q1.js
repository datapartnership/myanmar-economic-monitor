...
var start_period = ee.Date('2022-01-01'); // First MOD/MYD 13Q1 data 24 Feb 2000, 4 Jul 2002
var end_period = ee.Date('2023-04-05'); // Last MOD/MYD 13Q1 data 18 Feb 2022, 10 Feb 2022
...
var cropmask = function(img){
  return img.updateMask(cropland);
};
var mxd13q1 = mxd13q1_cleaned_sorted.map(cropmask);
...
//   image: mxd13q1.select('EVI').first().clip(aoi).unmask(-32768, false).toInt16(),
  ...
  .evaluate(function (indexes){
    indexes.forEach(function (index) {
      var image = mxd13q1
        ...
        .first();
      var filename = 'mmr_phy_mxd13q1_evi_' + index.replace(/_(\d{4})_(\d{2})_(\d{2})$/, '_$1$2$3');
      Export.image.toDrive({
        image: image.select('EVI').clip(aoi).unmask(-32768, false).toInt16(),
        description: filename,
        folder:'mmr_mxd13q1_evi',
        scale: 250,
        maxPixels: 1e13,
        region: aoi,
        crs: 'EPSG:4326'
