...
mmr_adm0 = gpd.read_file(
    DATA_DIR / "boundaries/mmr_polbnda_adm0_250k_mimu_20240215.shp"
)
mmr_adm1 = gpd.read_file(
    DATA_DIR / "boundaries/mmr_polbnda_adm1_250k_mimu_20240215.shp"
)
mmr_adm2 = gpd.read_file(
    DATA_DIR / "boundaries/mmr_polbnda_adm2_250k_mimu_20240215.shp"
)
mmr_adm3 = gpd.read_file(
    DATA_DIR / "boundaries/mmr_polbnda_adm3_250k_mimu_20240215.shp"
)
mmr_adm4 = gpd.read_file(
    DATA_DIR / "boundaries/mmr_polbnda_adm4_250k_mimu_20240215.shp"
)
mmr_sez = gpd.read_file(
    DATA_DIR / "boundaries/industrial__special_economic_zones_sept2019.shp"
)
...
for products in [Product.VNP46A3]:
    ...
    for admin_level, mmr_gdf in zip(
        ...
        for gdf_in, gdf_out, prefix in [
            ...
            if extr_in is not None:
                extracted = extracted.merge(
                    extr_in, on=["_merge_id", "date"], how="left"
                )
            else:
            ...
            if extr_out is not None:
                extracted = extracted.merge(
                    extr_out, on=["_merge_id", "date"], how="left"
                )
            else:
        ...
        out_path = (
            DATA_DIR / f"ntl/collection2/{folder}/ntl_mmr_adm{admin_level}_{folder}.csv"
        )
