...
def annual_ntl_pct_change(
    ...
    if date_col and date_col in work.columns:
        ...
        if detected_year_col is None:
            raise ValueError(
                "Could not determine year column from data; provide date_col or year_col"
            )

    # Aggregate by year to ensure one value per year
    agg = (
        work[[detected_year_col, value_col]]
        .dropna()
        .groupby(detected_year_col, as_index=False)[value_col]
        .mean()
    ...
    if mode == "yoy":
        ...
        agg["pct_change"] = agg.apply(
            lambda r: safe_pct(r[value_col], r["prev_year_value"]), axis=1
        )
    ...
    if mode == "baseline_series":
        if baseline_years is None:
            raise ValueError(
                "baseline_series mode requires baseline_years (tuple/list/range)"
            )
        ...
        if isinstance(baseline_years, range):
            ...
            raise ValueError(
                "baseline_years must be a tuple(start,end), list, set, or range"
            )
        ...
        baseline_period = (
            ...
            if len(years_list) > 1
            else str(years_list[0])
        )

        def safe_pct(curr, basev):
            try:
                return (
                    (curr - basev) / basev * 100
                    if pd.notnull(basev) and basev != 0
                    else float("nan")
                )
            except Exception:
                return float("nan")
        ...
        out["pct_change"] = out.apply(
            lambda r: safe_pct(r[value_col], baseline_val), axis=1
        )
        return out[
            ["year", value_col, "baseline_period", "baseline_value", "pct_change"]
        ]
...
def monthly_ntl_yoy(
    ...
    monthly = (
        work[["year", "month", "date", value_col]]
        .dropna()
    ...
    if baseline_year is None:
        ...
        merged = monthly.merge(
            prev[["year", "month", "prev_year_value"]], on=["year", "month"], how="left"
        )
        ...
        merged = merged[
            ["date", "year", "month", value_col, "prev_year_value", "pct_change"]
        ].sort_values("date")
        return merged
    else:
        base = monthly.loc[
            monthly["year"] == baseline_year, ["month", value_col]
        ...
        merged["pct_change"] = merged.apply(
            lambda r: safe_pct(r[value_col], r["baseline_value"]), axis=1
        )
        merged = merged[
            ["date", "year", "month", value_col, "baseline_value", "pct_change"]
        ].sort_values("date")
        return merged
...
def subnational_pct_change(
    ...
    *,
    ...
    if date_col and date_col in work.columns:
        ...
        if detected_year_col is None:
            raise ValueError(
                "Could not determine year column; provide date_col or year_col"
            )

    # Aggregate to one value per region-year
    agg = (
        work[[region_col, detected_year_col, value_col]]
        .dropna()
        .groupby([region_col, detected_year_col], as_index=False)[value_col]
        .mean()
    ...
    if current_year is None:
        ...
        curr = agg.loc[
            agg["year"] == int(current_year), [region_col, "year", value_col]
        ].rename(columns={value_col: "current_value"})
        curr = curr.assign(current_year=curr["year"]).drop(columns=["year"])

    base = agg.loc[agg["year"] == int(baseline_year), [region_col, value_col]].rename(
    ...
    out["pct_change"] = out.apply(
        lambda r: safe_pct2(r["current_value"], r["baseline_value"]), axis=1
    )
...
def calculate_percentage_change(
    ...
    if mode == "yoy":
        ...
        work_df[f"{value_col}{suffix}"] = (
            (work_df[value_col] - work_df["prev_year_value"])
            / work_df["prev_year_value"]
            * 100
        )
        return work_df[
            group_cols + ["year", value_col, "prev_year_value", f"{value_col}{suffix}"]
        ].dropna()
        ...
        work_df[f"{value_col}{suffix}"] = (
            (work_df[value_col] - work_df["prev_month_value"])
            / work_df["prev_month_value"]
            * 100
        )
        return work_df[
            group_cols
            + [date_col, value_col, "prev_month_value", f"{value_col}{suffix}"]
        ].dropna()
        ...
        baseline_values = (
            baseline_data.groupby(group_cols)[value_col]
            .mean()
            .reset_index()
            .rename(columns={value_col: "baseline_value"})
        )
        ...
        comparison_values = (
            comparison_data.groupby(group_cols)[value_col]
            .mean()
            .reset_index()
            .rename(columns={value_col: "current_value"})
        )
...
def subnational_monthly_pct_change(
    ...
    *,
    ...
    if date_col and date_col in work.columns:
        ...
        if year_col not in work.columns or month_col not in work.columns:
            raise ValueError(
                "Must provide date_col or have both year_col and month_col in data"
            )
        work["year"] = work[year_col].astype(int)
        work["month"] = work[month_col].astype(int)
        work["date"] = pd.to_datetime(
    ...
    agg = (
        work[[region_col, "year", "month", "date", value_col]]
        .dropna()
        .groupby([region_col, "year", "month"], as_index=False)
        .agg({"date": "first", value_col: "mean"})
    )
    ...
    baseline_data = agg[agg["year"] == baseline_year][
        [region_col, "month", value_col]
    ].copy()
    ...
    result = comparison_data.merge(baseline_data, on=[region_col, "month"], how="left")
    ...
    result["pct_change"] = result.apply(
        lambda row: safe_pct_change(row[value_col], row["baseline_value"]), axis=1
    )
