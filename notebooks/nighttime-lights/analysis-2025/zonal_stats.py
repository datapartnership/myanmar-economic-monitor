...
def process_admin_level(shapefile_path, admin_level, admin_column, admin_pcode_column):
    output_stats_path = os.path.join(
        output_dir, f"Myanmar_Admin{admin_level}_Daily_Nightlights_{date_range}.csv"
    )
    ...
    try:
        ...
        gdf = gpd.read_file(shapefile_path)[
            [admin_column, admin_pcode_column, "geometry"]
        ]
                ...
                with rasterio.open(tif_file) as src:
                    ...
                    if np.any(data < 0):
                        ...
                        temp_tif = os.path.join(
                            ...
                            f"temp_{os.path.basename(tif_file)}",
                        )
                        profile = src.profile
        ...
        if all_results:
            ...
            if "sum" in results_df.columns:
                ...
                print(
                    f"Keeping {non_zero_sum.sum()} of {len(results_df)} rows with sum > 1"
                )
            ...
            if "count" in results_df.columns:
                ...
                for name, group in results_df.groupby(group_cols):
                    if group["count"].sum() > 0:  # Avoid division by zero
                        ...
                        weighted_mean = (group["mean"] * group["count"]).sum() / group[
                            "count"
                        ].sum()
                    else:
            ...
            print(
                f"Reduced from {len(results_df)} to {len(aggregated_df)} rows after aggregation"
            )
            ...
            print(
                f"Tiles processed: {len(set(','.join(aggregated_df['tile']).split(',')))}"
            )
            if "sum" in aggregated_df.columns:
                print(
                    f"Sum range: {aggregated_df['sum'].min()} to {aggregated_df['sum'].max()}"
                )
...
process_admin_level(admin2_shapefile, 2, "ADM2_EN", "ADM2_PCODE")
