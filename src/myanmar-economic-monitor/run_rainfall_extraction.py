...
def main():
    ...
    adm0_file = extractor.save_to_csv(
        national_rainfall,
        "myanmar_rainfall_adm0_monthly.csv"
    )
    print(f"✓ Saved ADM0 data: {adm0_file}")
    print(f"  Records: {len(national_rainfall)}")
    print(f"  Date range: {national_rainfall['date'].min()} to {national_rainfall['date'].max()}")
